/**
 * Canvas UI: Hex Float — vanilla-порт React-компонента.
 *
 * Страница превращается в пол из глянцевых гексов, отклонённый в перспективе:
 * лист лежит на столе. Курсор впрыскивает жидкость, которая разглаживает
 * плитки в читаемое окно и затягивается, когда курсор уходит.
 *
 * Основной режим требует экспериментального HTML-in-canvas API
 * (`<canvas layoutsubtree>` + `ctx.drawElementImage`). Если его нет,
 * `supportsHtmlInCanvas()` вернёт false и вызывающий код не должен
 * инициализировать эффект вовсе.
 */

export interface HexFloatOptions {
  /** Ширина одной плитки в CSS-пикселях. */
  size?: number;
  /** Шов между плитками в CSS-пикселях. */
  gap?: number;
  /** Ширина блестящей фаски в CSS-пикселях. */
  bevel?: number;
  /** Наклон страницы назад в градусах (-30..30). Плюс — верх уходит от зрителя. */
  tilt?: number;
  /** Близость камеры (0..1). Больше — сильнее перспектива наклона. */
  perspective?: number;
  /** Насколько плитки покачиваются (0..1). 0 — стоят неподвижно. */
  float?: number;
  /** Скорость покачивания. 1 — обычная. */
  speed?: number;
  /** Сила бликов на фасках и гранях (0..2). */
  shine?: number;
  /** Насколько плитки приподнимаются по краю читаемого окна (0..1). */
  lift?: number;
  /** Размер всплеска жидкости от курсора в CSS-пикселях. */
  radius?: number;
  /** Насколько движение курсора толкает жидкость (0..3). */
  flow?: number;
  /** Завихрённость жидкости (0..15). */
  swirl?: number;
  /** Насколько долго держится след до затягивания (0..1). */
  trail?: number;
  /** Сила радужного сдвига оттенка на бликах (0..2). 0 — нейтральные блики. */
  iridescence?: number;
  /** Свечение вокруг ярких бликов (0..1). 0 — проход пропускается. */
  bloom?: number;
  /** Анимированное зерно поверх кадра (0..1). 0 — проход пропускается. */
  grain?: number;
  /** Цвет шва как [r, g, b] в диапазоне 0..1, либо "auto" — вывести из фона. */
  gapColor?: [number, number, number] | 'auto';
}

export interface HexFloatElements {
  /** Canvas с layoutsubtree, внутри которого живёт HTML. */
  source: HTMLCanvasElement;
  /** Элемент внутри source-canvas, который снимается. */
  content: HTMLElement;
  /** Canvas, в который рисует WebGL. */
  output: HTMLCanvasElement;
}

export interface HexFloatInstance {
  /** Обновить параметры на лету. */
  setOptions: (options: HexFloatOptions) => void;
  /** Перечитать размеры. Вызывать при ресайзе. */
  resize: () => void;
  /** Остановить цикл и освободить GPU-ресурсы. */
  destroy: () => void;
}
